...
pub fn generate() -> Result<(), String> {
    ...
    let rs_tool_path = join_paths([ env::var("OUT_DIR").unwrap(), String::from("rstool.py")]).unwrap();
    fs::write(&rs_tool_path, &rstool_bytes).expect(&format!("couldn't write rstool.py tool in {:?}", &*rs_tool_path));
    ...
    let cmd = Command::new("python3")
        ...
        .env("PYTHONPATH", join_paths([ env::var("OUT_DIR").unwrap_or(String::new()) , vmodtool_dir ]).unwrap())
        .output()
